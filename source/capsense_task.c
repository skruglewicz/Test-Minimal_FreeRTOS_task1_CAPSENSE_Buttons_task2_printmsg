...
//    slider_touched = slider_touch->numPosition;
    ...
    {
        print_cmd_data.command = PRINT_MESSAGE;
       // printf("button0 touched   Value: %d\r\n", button0_status);
        strcpy(print_cmd_data.message, "button0 touched");
        send_print_command = true;
    }
    ...
    {
        print_cmd_data.command = PRINT_MESSAGE;
        strcpy(print_cmd_data.message, "button1 touched");
        send_print_command = true;
    }
...
//    {
//        print_cmd_data.command = LED_UPDATE_BRIGHTNESS;
//
//        /* brightness in percentage */
//        print_cmd_data.brightness = (slider_pos * 100)
//                    / cy_capsense_context.ptrWdConfig[CY_CAPSENSE_LINEARSLIDER0_WDGT_ID].xResolution;
//        send_print_command = true;
//    }

    /* Send command with message to the Print Command QUEUE */
    if(send_print_command)
    {
        xQueueSendToBack(print_command_data_q, &print_cmd_data, 0u);
    }
